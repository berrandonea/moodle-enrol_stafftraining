 ...
 * enrol_stafftraining external functions.
 */
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later 
 */
class enrol_stafftraining_external extends external_api {
    ...
    /**
     * Return stafftraining-enrolment instance information.
     *
     * @param int $instanceid instance id of stafftraining enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve stafftraining enrolment plugin.
        $enrolplugin = enrol_get_plugin('stafftraining');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of stafftraining enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * Enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the stafftraining enrolment plugin.
        $enrol = enrol_get_plugin('stafftraining');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_stafftraining');
        }

        // We can expect multiple stafftraining-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "stafftraining") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_stafftraining_enrol($instance);
            if ($enrolstatus === true) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_self_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_stafftraining')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_stafftraining', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_stafftraining($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
