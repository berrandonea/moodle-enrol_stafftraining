...
/**
 * stafftraining enrolment plugin implementation.
 * @author Brice Errandonea
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_stafftraining_plugin extends enrol_plugin {

    protected $lasternoller = null;
    protected $lasternollerinstanceid = 0;

    public function askforvalidation() {
		return true;
	}
	
	public function yourrequests() {
		global $DB, $USER;
		$requests = $DB->get_records('enrol_stafftraining_enroldata', array('userid' => $USER->id, 'recorded' => 1));
		return $requests;
	}

	public function othersrequests() {
		global $DB, $USER;
		$requests = $DB->get_records('enrol_stafftraining_enroldata', array('chiefid' => $USER->id, 'recorded' => 1));
		$mygpeecassignments = $DB->get_records('role_assignments', array('roleid' => 12, 'userid' => $USER->id));
		foreach ($mygpeecassignments as $mygpeecassignment) {
			$mygpeeccontext = $DB->get_record('context', array('id' => $mygpeecassignment->contextid), '*', MUST_EXIST);
			$mygpeectraining = $DB->get_record('course', array('id' => $mygpeeccontext->instanceid), '*', MUST_EXIST);
			$mygpeectraininggroups = $DB->get_records('groups', array('courseid' => $mygpeectraining->id));
			foreach ($mygpeectraininggroups as $mygpeectraininggroup) {
				$grouprequests = $DB->get_records('enrol_stafftraining_enroldata', array('groupid' => $mygpeectraininggroup->id, 'recorded' => 1));
				$requests = array_merge($requests, $grouprequests);
			}
		}
		return $requests;
	}
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_stafftraining_enrol($instance, false) !== true) {
                ...
                // would hide stafftraining-enrolment icons from guests.
                continue;
            }
        ...
        $icons = array();
        //~ if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_stafftraining'), 'enrol_stafftraining');
        //~ }
        //~ if ($key) {
            //~ $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_stafftraining'), 'enrol_stafftraining');
        //~ }
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_stafftraining_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/stafftraining:config', $context)) {
            return false;
        }
     ...
     */
    public function enrol_stafftraining(stdClass $instance, $data = null) {
        global $DB, $USER, $CFG;
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/stafftraining/locallib.php");

        $enrolstatus = $this->can_stafftraining_enrol($instance);

        if (true === $enrolstatus) {
            // This user can enrol himself using this instance.
            $form = new enrol_stafftraining_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_stafftraining($instance, $data);
                }
            }
        } else {
            // This user can not enrol himself using this instance. Using an empty form to keep
            // the UI consistent with other enrolment plugins that returns a form.
            $data = new stdClass();
            $data->header = $this->get_instance_name($instance);
            $data->info = $enrolstatus;

            // The can_stafftraining_enrol call returns a button to the login page if the user is a
            // guest, setting the login url to the form if that is the case.
            $url = isguestuser() ? get_login_url() : null;
            $form = new enrol_stafftraining_empty_form($url, $data);
        }
    ...
    /**
     * Checks if user can ask for enrolment using the stafftraining method.
     *
     * @param stdClass $instance enrolment instance
     ...
     */
    public function can_stafftraining_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            ...
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_stafftraining');
            }
        }
        ...
        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_stafftraining', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_stafftraining', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more such enrolments here.
                return get_string('maxenrolledreached', 'enrol_stafftraining');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_stafftraining', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_stafftraining_enrol($instance);

        //~ if ($instance->password) {
            //~ $instanceinfo->requiredparam = new stdClass();
            //~ $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_stafftraining');
        //~ }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_stafftraining_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_stafftraining', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_stafftraining', format_string($course->fullname, true, array('context'=>$context)));
    ...
    /**
     * Enrol stafftraining cron support.
     * @return void
     */
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying stafftraining-enrolments...');
        ...
        //       and that user accessed course at least once too (=== user_lastaccess record exists). Peut-on en dire autant pour enrol_stafftraining ?

        // First deal with users that did not log in for a really long time - they do not have user_lastaccess records.
        $sql = "SELECT e.*, ue.userid
                  FROM {user_enrolments} ue
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'stafftraining' AND e.customint2 > 0)
        ...
        $trace->output('...user stafftraining-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for stafftraining enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/stafftraining:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/stafftraining:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/stafftraining:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(new pix_icon('t/delete', ''), get_string('unenrol', 'enrol'), $url, array('class'=>'unenrollink', 'rel'=>$ue->id));
        }
        if ($this->allow_manage($instance) && has_capability("enrol/stafftraining:manage", $context)) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent stafftraining enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual, stafftraining or self enrol.
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/stafftraining:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/stafftraining:config', $context)) {
            return false;
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('text', 'customint1', get_string('capacity', 'enrol_stafftraining'));
        $mform->setType('customint1', PARAM_INT);
        $mform->setDefault('customint1', $instance->customint1);


        //~ $options = $this->get_status_options();
        //~ $mform->addElement('select', 'status', get_string('status', 'enrol_stafftraining'), $options);
        //~ $mform->addHelpButton('status', 'status', 'enrol_stafftraining');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_stafftraining'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_stafftraining');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        //~ $passattribs = array('size' => '20', 'maxlength' => '50');
        //~ $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_stafftraining'), $passattribs);
        //~ $mform->addHelpButton('password', 'password', 'enrol_stafftraining');
        ...
        //~ $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_stafftraining'), $options);
        //~ $mform->addHelpButton('customint1', 'groupkey', 'enrol_stafftraining');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_stafftraining'), $roles);



        //~ $options = array('optional' => true, 'defaultunit' => 86400);
        //~ $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_stafftraining'), $options);
        //~ $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_stafftraining');
        ...
        //~ $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_stafftraining'), $options);
        //~ $mform->setDefault('enrolstartdate', 0);
        //~ $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_stafftraining');

        //~ $options = array('optional' => true);
        //~ $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_stafftraining'), $options);
        //~ $mform->setDefault('enrolenddate', 0);
        //~ $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_stafftraining');

        //~ $options = $this->get_longtimenosee_options();
        //~ $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_stafftraining'), $options);
        //~ $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_stafftraining');

        //~ $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_stafftraining'));
        //~ $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_stafftraining');
        ...
        //~ }
        //~ if (count($cohorts) > 1) {
            //~ $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_stafftraining'), $cohorts);
            //~ $mform->addHelpButton('customint5', 'cohortonly', 'enrol_stafftraining');
        //~ } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_stafftraining'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_stafftraining');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_stafftraining'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_stafftraining');
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        //~ if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            //~ if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                //~ $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_stafftraining');
            //~ }
        //~ }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for stafftraining-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for stafftraining-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/stafftraining:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/stafftraining:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_stafftraining_get_fontawesome_icon_map() {
    return [
        'enrol_stafftraining:withkey' => 'fa-key',
        'enrol_stafftraining:withoutkey' => 'fa-sign-in',
    ];
}
